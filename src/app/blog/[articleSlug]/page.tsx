...
export async function generateMetadata({ params }: { params: Promise<{ articleSlug: string }> }): Promise<Metadata> {
  ...
  return {
    title,
    description,
    openGraph: { title, description, type: 'article', url: `${siteUrl}/blog/${articleSlug}`, ...(ogImage ? { images: [{ url: ogImage }] } : {}) },
    twitter: { card: ogImage ? 'summary_large_image' : 'summary', title, description, ...(ogImage ? { images: [ogImage] } : {}) },
    alternates: { canonical: `${siteUrl}/blog/${articleSlug}` },
  };
}
...
export default async function BlogArticlePage({ params }: { params: Promise<{ articleSlug: string }> }) {
  const { articleSlug } = await params;
  const article = await getArticle(articleSlug);
  if (!article) notFound();
    ...
    <main>
      <section className="bg-[#0C3290] py-14 px-5">
        <div className="max-w-[700px] mx-auto">
          <Link href="/blog" className="text-xs text-white/40 hover:text-white/70 transition-colors mb-4 inline-block">← ブログ一覧に戻る</Link>
          ...
          {article.sections.map((section, i) => (
            <div key={i}>
              <h2 className="text-lg font-bold text-[#0C3290] mb-3 flex items-center gap-3" style={{ fontFamily: '"Noto Sans JP", sans-serif' }}>
                <span className="w-7 h-7 rounded-full bg-amber-500 text-[#0C3290] font-bold flex items-center justify-center text-xs flex-shrink-0">{i + 1}</span>
                {section.heading}
              </h2>
      ...
      <section className="py-12 px-5 bg-[#0C3290]">
        <div className="max-w-[500px] mx-auto text-center text-white">
          ...
          <Link href="/blog" className="inline-block px-8 py-3 bg-amber-500 text-[#0C3290] font-bold rounded-xl text-sm hover:bg-amber-600 transition-colors">ブログ一覧 →</Link>
        </div>
      </section>
    </main>
  );
