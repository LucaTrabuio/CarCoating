...
export default function StoreCasesPage() {
  ...
  useEffect(() => {
    fetch('/api/stores')
      ...
      .then((data: StoreData[]) => {
        ...
        if (found) setDiscountRate(found.discount_rate || 20);
      })
      .catch(() => {});
  }, [storeId]);
        ...
        <div className="max-w-[1100px] mx-auto">
          ...
          <div className="grid md:grid-cols-2 gap-6 items-start">
            ...
            <div>
              ...
              <Link href={`/${storeId}/booking?plan=${featured.tierId}`}
      ...
      <section className="py-10 px-5">
        <div className="max-w-[600px] mx-auto bg-[#0f1c2e] rounded-xl p-8 text-center text-white">
          ...
          <div className="flex gap-3 justify-center flex-wrap">
            <Link href={`/${storeId}/booking`} className="px-5 py-2.5 bg-white text-[#0f1c2e] font-bold rounded-lg text-sm">予約する →</Link>
            <Link href={`/${storeId}/booking?mode=inquiry`} className="px-5 py-2.5 bg-amber-500 text-white font-bold rounded-lg text-sm">お問い合わせ →</Link>
          </div>
        </div>
      </section>
    </main>
