...
export default async function V3BlogArticlePage({ params }: { params: Promise<{ storeId: string; slug: string }> }) {
    ...
    <main>
      <section className="bg-[#0f1c2e] py-14 px-5">
        <div className="max-w-[700px] mx-auto">
          ...
          <div className="flex items-center gap-2 mb-3">
            <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${categoryColors[article.category]}`}>{categoryLabels[article.category]}</span>
            <span className="text-[10px] text-white/30">{article.publishDate}</span>
          </div>
          <h1 className="text-white text-xl md:text-2xl font-bold leading-snug" style={{ fontFamily: '"Noto Serif JP", serif' }}>{article.title}</h1>
      ...
      <section className="py-12 px-5">
        <div className="max-w-[700px] mx-auto space-y-10">
          {article.content.map((section, i) => (
            <div key={i}>
              <h2 className="text-lg font-bold text-[#0f1c2e] mb-3 flex items-center gap-3" style={{ fontFamily: '"Noto Serif JP", serif' }}>
                <span className="w-7 h-7 rounded-full bg-amber-500 text-white font-bold flex items-center justify-center text-xs flex-shrink-0">{i + 1}</span>
                {section.heading}
              </h2>
      ...
      <section className="py-12 px-5 bg-[#0f1c2e]">
        <div className="max-w-[500px] mx-auto text-center text-white">
          ...
          <Link href={`${base}/booking`} className="inline-block px-8 py-3 bg-amber-500 text-white font-bold rounded-xl text-sm hover:bg-amber-600 transition-colors">ご予約・お問い合わせ →</Link>
        </div>
      </section>
    </main>
  );
