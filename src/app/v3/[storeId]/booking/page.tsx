...
function V3BookingContent() {
  const { storeId } = useParams<{ storeId: string }>();
  const searchParams = useSearchParams();
  const mode = searchParams.get('mode');
  const isInquiry = mode === 'inquiry';

  const [store, setStore] = useState<V3StoreData | null>(null);
  ...
  return (
    <main>
      <section className="bg-[#0f1c2e] py-12 px-5 text-center">
        ...
        {store && <p className="text-white/40 text-sm mt-1">{store.store_name}</p>}
      </section>
      ...
      <section className="py-10 px-5">
        <div className="max-w-[700px] mx-auto">
          {!isInquiry && (
            <div className="mb-8">
              <h2 className="text-lg font-bold text-[#0f1c2e] mb-4" style={{ fontFamily: '"Noto Serif JP", serif' }}>希望日時を選択</h2>
              <BookingCalendar />
            </div>
          )}
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <button type="submit"
              className="w-full py-3 bg-gradient-to-br from-amber-600 via-amber-500 to-amber-700 text-white font-bold rounded-lg text-sm hover:opacity-90 transition-opacity cursor-pointer">
