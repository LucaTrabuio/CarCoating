...
function getStoreCases(domainKey: string): CaseEntry[] {
  const domain = SUB_COMPANY_TO_DOMAIN[domainKey];
  if (domain) {
...
export function CasesBody({ basePath, domainKey }: CasesBodyProps) {
  const [filterCoating, setFilterCoating] = useState('all');
  const [filterCar, setFilterCar] = useState('all');

  const cases = getStoreCases(domainKey);
      ...
      <section className="py-10 px-5 bg-[#0C3290]">
        <div className="max-w-[500px] mx-auto text-center text-white">
          ...
          <div className="flex gap-3 justify-center">
            <Link href={`${basePath}/booking`} className="px-6 py-2.5 bg-amber-500 text-[#0C3290] font-bold rounded-md text-sm hover:bg-amber-500">予約する →</Link>
            <Link href={`${basePath}/booking?mode=inquiry`} className="px-6 py-2.5 bg-white/10 border border-white/15 text-white font-semibold rounded-md text-sm hover:bg-white/20">お問い合わせ</Link>
          </div>
        </div>
      </section>
    </main>
