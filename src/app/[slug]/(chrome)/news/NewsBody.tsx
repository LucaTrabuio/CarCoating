...
export function NewsBody({ store, basePath }: NewsBodyProps) {
  const newsItems = parseStoreNews(store.store_news);
    ...
    <main>
      <section className="bg-[#0C3290] py-6 md:py-12 px-5 text-center">
        <div className="max-w-[800px] mx-auto">
          <Link href={basePath} className="text-xs text-white/40 hover:text-white/70 transition-colors mb-3 inline-block">← トップに戻る</Link>
          <h1 className="text-white text-2xl font-bold" style={{ fontFamily: 'var(--site-font, "Noto Sans JP", sans-serif)' }}>ニュース・お知らせ</h1>
        </div>
      </section>
      ...
      <section className="py-10 px-5 bg-[#0C3290]">
        <div className="max-w-[500px] mx-auto text-center text-white">
          ...
          <div className="flex gap-3 justify-center">
            <Link href={`${basePath}/coatings`} className="px-5 py-2.5 bg-white/10 border border-white/15 text-white font-semibold rounded-md text-sm hover:bg-white/20">メニュー一覧</Link>
            <Link href={`${basePath}/booking`} className="px-5 py-2.5 bg-amber-500 text-[#0C3290] font-bold rounded-md text-sm hover:bg-amber-500">ご予約 →</Link>
          </div>
        </div>
      </section>
    </main>
