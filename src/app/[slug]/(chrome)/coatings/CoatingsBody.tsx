/*
 * CoatingsBody — extracted render body for the coatings menu page.
 * Used by both the flat /{slug}/coatings page and the nested
 * /{slug}/{subSlug}/coatings page.
 */
...
export function CoatingsBody({ store, basePath, discountRate, coatingTiers }: CoatingsBodyProps) {
          ...
          <section key={tier.id} id={tier.id} className={`py-12 px-5 ${isEven ? 'bg-slate-50/80' : 'bg-white'}`}>
            <div className="max-w-[900px] mx-auto border-l-4 border-l-amber-500 pl-6">
                    ...
                    <tr className="bg-white border-b border-slate-100">
                      ...
                      {ALL_SIZES.map(size => (
                        <td key={size} className="px-2.5 py-2 text-center text-[11px] text-slate-400 line-through">
                          {isBlurred(tier.id, 'web_price', blurFields) ? (
                            <Link href={`${basePath}/inquiry?tier=${tier.id}&prefill=price`} className="text-blue-500 hover:text-blue-700 font-semibold">要問合せ</Link>
              ...
              <div className="flex gap-2.5 mt-5 flex-wrap">
                <Link href={`${basePath}/price`} className="px-5 py-2 bg-amber-500 text-[#0C3290] font-bold rounded-md text-[13px] hover:bg-amber-500 transition-colors">見積もりシミュレーター →</Link>
                <Link href={`${basePath}/booking`} className="px-5 py-2 bg-slate-200 text-slate-700 font-bold rounded-md text-[13px] hover:bg-slate-300 transition-colors">予約する</Link>
                <Link href={`${basePath}/inquiry?tier=${tier.id}`} className="px-5 py-2 bg-blue-600 text-white font-bold rounded-md text-[13px] hover:bg-blue-700 transition-colors">お問い合わせ</Link>
              </div>
            </div>
          </section>
        );
      ...
      <section className="py-14 px-5 bg-[#0C3290]">
        <div className="max-w-[520px] mx-auto text-center text-white">
          ...
          <div className="flex gap-3 justify-center">
            <Link href={`${basePath}/price`} className="px-6 py-2.5 bg-amber-500 text-[#0C3290] font-bold rounded-md text-sm hover:bg-amber-500 transition-colors">見積もりシミュレーター →</Link>
            <Link href={`${basePath}/booking`} className="px-6 py-2.5 bg-white/10 border border-white/15 text-white font-semibold rounded-md text-sm hover:bg-white/20 transition-colors">ご予約</Link>
          </div>
        </div>
      </section>
    </main>
