      ...
      <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/85 via-black/40 to-transparent p-8 pb-8">
        <div className="max-w-[900px] mx-auto text-center">
          ...
          <div className="flex gap-3 justify-center mt-5">
            {config.show_cta_booking && <TrackedLink href={`${basePath}/booking`} storeId={store.store_id} event="cta_booking" className="px-7 py-3 bg-amber-500 text-black font-bold rounded-lg text-sm hover:bg-amber-500 transition-colors">{'\u4E88\u7D04\u3059\u308B'}</TrackedLink>}
            {config.show_cta_inquiry && <TrackedLink href={`${basePath}/booking?mode=inquiry`} storeId={store.store_id} event="cta_inquiry" className="px-7 py-3 bg-white border border-white text-[#0C3290] font-semibold rounded-lg text-sm hover:bg-gray-100 transition-colors">{'\u304A\u554F\u3044\u5408\u308F\u305B'}</TrackedLink>}
          </div>
        </div>
      </div>
    </section>
