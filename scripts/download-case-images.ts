/**
 * Download all construction case images from franchise sites to local disk.
 * These sites will close, so we need to preserve the images.
 *
 * Downloads to: downloaded-case-images/{domain}/{filename}
 * Creates manifest: downloaded-case-images/manifest.json
 *
 * Run: npx tsx scripts/download-case-images.ts
 */
...
// All image URLs from update-case-images.ts DOMAIN_IMAGES
...
const OUTPUT_DIR = join(import.meta.dirname || __dirname, '..', 'downloaded-case-images');
    ...
    const request = client.get(url, { timeout: 15000 }, (res) => {
      ...
      res.on('end', () => {
        ...
        if (buffer.length < 1000) {
          console.log(`    SKIP (too small ${buffer.length}b): ${url}`);
          resolve(false);
          return;
        }
...
async function main() {
  ...
  const manifest: Record<string, { url: string; localPath: string; filename: string }[]> = {};
  let downloaded = 0;
  let failed = 0;
  let skipped = 0;

  const domains = Object.keys(DOMAIN_IMAGES);
  console.log(`Downloading images from ${domains.length} domains...\n`);

  for (const domain of domains) {
    const images = DOMAIN_IMAGES[domain];
    ...
    for (const url of images) {
      ...
      const destPath = join(domainDir, filename);

      if (existsSync(destPath)) {
        manifest[domain].push({ url, localPath: `${domain}/${filename}`, filename });
        skipped++;
        continue;
      }
