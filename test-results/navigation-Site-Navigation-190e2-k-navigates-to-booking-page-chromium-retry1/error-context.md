...
- Name: navigation.spec.ts >> Site Navigation >> Header navigation (desktop) >> header nav link navigates to booking page
- Location: tests/navigation.spec.ts:58:9

# Error details

```
Error: locator.click: Error: strict mode violation: locator('nav a[href="/eniwa/booking"]') resolved to 2 elements:
    1) <a href="/eniwa/booking" class="text-gray-600 text-[13px] hover:text-black transition-colors">ご予約</a> aka getByRole('banner').getByRole('link', { name: 'ご予約' })
    2) <a href="/eniwa/booking" class="hover:text-white transition-colors">ご予約</a> aka getByRole('contentinfo').getByRole('link', { name: 'ご予約' })

Call log:
  - waiting for locator('nav a[href="/eniwa/booking"]')
...
> 60  |       await page.locator('nav a[href="/eniwa/booking"]').click();
  ...
  150 |   });
  151 | 
  152 |   test.describe('Quiz popup', () => {
  153 |     test('quiz popup appears after scroll on store page', async ({ page }) => {
  154 |       await page.goto('/eniwa');
  155 |       // Clear any stored dismissal so popup can appear
  156 |       await page.evaluate(() => localStorage.removeItem('quiz_popup_dismissed'));
  157 |       await page.reload();
  158 |       // Scroll down to trigger the popup
  159 |       await page.evaluate(() => window.scrollTo(0, document.body.scrollHeight));
